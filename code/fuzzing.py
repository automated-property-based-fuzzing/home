...
class Fuzzing(object):
            ...
            while self.now_event<self.event_num:
                ...
                if dmfnum>-1:
                    ...
                    if execute_result == "True":
                        ...
                        self.f_event.write("fail"+"::"+execute_result+"\n\n")
                        self.f_event.flush()
                ...
                #Otherwise, select an event to execute according to the exploration rules
                    ...
                    if execute_result == True:
                        ...
                        #Record event
                        if event.view!=None:
                            self.f_event.write(str(self.now_event)+"::"+event.action+"::"+event.text+"::"+event.view.line.strip()+"::"+event.view.xpath+"\n")
                        else:
    ...
    def execute_dmf(self,dmfnum,f_dmf):
        ...
        #     widget.real_text=""
        start_screen = self.device.screen.lines
        f_dmf.write(str(self.now_event)+"::dmf::"+nowdmf.name+"::start\n")
        print("********************************")
        inputtext = ""
        now_layout = 0
        for widget in nowdmf.widgets:
        ...
        for event in nowdmf.events:
            ...
            for widget in nowdmf.widgets:
                if widget.name == event.widget:
                    ...
                    try:
                        view=self.findviewinlayout(self.device.screen.lines, widget, None,"same")
                        if view == None:
            ...
            view=View(line,[])
            view.instance = nowwidget.instance+""
            ...
            #If the execution is successful:
                ...
                if execute_event.view!=None:
                    self.f_event.write(str(self.now_event)+"::"+execute_event.action+"::"+execute_event.text+"::"+execute_event.view.line.strip()+"::"+execute_event.view.xpath+"\n")
                else:
        ...
        for postcondition in nowdmf.postconditions:
            ...
            if postcondition.datatype=="":
                view_textin=self.findviewinlayout(self.device.screen.lines, postwidget, None, "in")
                view_textin_startscreen=self.findviewinlayout(start_screen, postwidget, None, "in")
                first_dataobjectnumber = self.getnumberbytext(start_screen, postwidget)
                end_dataobjectnumber = self.getnumberbytext(self.device.screen.lines, postwidget)
                view_textsame=self.findviewinlayout(self.device.screen.lines, postwidget, None, "same")
                widgetline=self.util.changewidgettoline(postwidget)
                findinfo = postcondition.relation+"::"+postwidget.real_text+"::"+widgetline
                #同步修改
                if postcondition.relation == "in":
                    if (nowdmf.type!="search" and view_textin==None and view_textin_startscreen==None) or (nowdmf.type=="search" and view_textin==None):
                        return findinfo
                elif postcondition.relation == "not in":
                    if view_textsame!=None and end_dataobjectnumber-first_dataobjectnumber!=1 and postwidget.real_text.strip()!="":
                        view_textsame=self.findviewinlayout(self.device.screen.lines, postwidget, None, "same")
                        return findinfo
            ...
            #The second type is to judge whether there is a widget with recorded text in a data list in the current interface
                ...
                while i<self.items:
                    ...
                    i=i+1
                view_textin=self.findviewinlayout(self.device.screen.lines, postwidget, postdatatypelist,"in")
                view_textsame=self.findviewinlayout(self.device.screen.lines, postwidget, postdatatypelist,"same")
                if postcondition.relation == "in" and view_textin==None:
                    return findinfo
                elif postcondition.relation == "not in" and view_textsame!=None:
                    return findinfo
                f_dmf.write(findinfo+"\n")
                f_dmf.flush()
    ...
    def getnumberbytext(self, now_layout, widget):
        view=None
        number = 0
        for line in now_layout:
            if "<node" not in line:
                continue
            view = View(line,[])
            if widget.real_text!=None and widget.real_text == view.text:
                number = number+1
            else:
                continue
        return number
    
    def findviewnum(self, now_layout, widget, datalist):
        viewnum =0
        for line in now_layout:
            if "<node" not in line:
                continue
    ...
    def findviewinlayout(self, now_layout, widget, datalist, inflag):
        view=None
        now_instance=0
        for line in now_layout:
            if "<node" not in line:
            ...
            if widget.className!="" and widget.className != view.className:
                ...
                continue
            elif datalist==None and widget.real_text!="":
                if inflag=="in" and widget.real_text not in view.text:
                    continue
                elif inflag=="same" and widget.real_text != view.text:
                    continue
            elif view.package != self.app.package_name:
                continue
                ...
                if precondition.datatype=="":
                    for widget in dmf.widgets:
                        if widget.name == precondition.widget:
                            perwidget = widget
                    view_textin=self.findviewinlayout(self.device.screen.lines, perwidget, None, "in")
                    view_textsame=self.findviewinlayout(self.device.screen.lines, perwidget, None, "same")
                    viewnum = self.findviewnum(self.device.screen.lines, perwidget, None)
                    if precondition.relation == "in" and view_textin==None:
                        conditionpass = False
                    if precondition.relation == "samein" and view_textsame==None:
                        conditionpass = False
                    elif precondition.relation == "not in" and view_textsame!=None:
                        conditionpass = False
                    elif "smaller" in precondition.relation:
                        item=precondition.relation.split("::")
                        if viewnum>int(item[1]):
                            conditionpass = False
                else:
                    predatatypelist=[]
                    i = 0 
                    ...
                    #The third type is to judge whether there is a widget with recorded text in a data list in the current interface
                        ...
                        for widget in dmf.widgets:
                            if widget.name == precondition.widget:
                                perwidget = widget
                        view_textin=self.findviewinlayout(self.device.screen.lines, perwidget, predatatypelist, "in")
                        view_textsame=self.findviewinlayout(self.device.screen.lines, perwidget, predatatypelist, "same")
                        if precondition.relation == "in" and view_textin==None:
                            conditionpass = False
                        elif precondition.relation == "not in" and view_textsame!=None:
                            conditionpass = False
