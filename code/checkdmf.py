...
class CheckRecord(object):
    ...
    def findviewnum(self,now_layout, widget):
        viewnum=0
        now_instance=0
    ...
    def findviewinlayout(self,now_layout, widget):
        ...
        for line in now_layout:
            ...
            if widget.className!="" and widget.className != view.className:
                ...
                continue
            elif widget.real_text!="" and widget.real_text not in view.text:
                continue
            elif widget.instance!="" and now_instance<int(widget.instance):
                now_instance=now_instance+1
            else:
    ...
    def execute_event(self,device,event,num):
        try:
            ...
            if event.action == "click":
                ...
                print("edit"+event.view.line+event.text+"\n")
        ...
        while(True):
            ...
            for precondition in nowdmf.preconditions:
                if precondition.datatype=="":
                    if precondition.datatype!="" and precondition.datatype!=datatype:
                        print("No matching data type")
                        continue
                    for widget in nowdmf.widgets:
                        if widget.name == precondition.widget:
                            perwidget = widget
                    view=self.findviewinlayout(self.now_layout, perwidget)
                    viewnum = self.findviewnum(self.now_layout, perwidget)
                    if precondition.relation == "in" and view==None:
                        conditionpass = False
                        print("Pre-check failed, "+perwidget.name+" does not exist")
                        break
                        ...
                        if viewnum>int(item[1]):
                            conditionpass = False
                            print("Pre-check failed, "+perwidget.name+" not "+precondition.relation)
                            break
