class View(object):
    ...
    def __init__(self, line, keywordlist):
        self.line = line
        #以后有时间重构一下，先阶段只能在每次找DMF的时候取消注释下面两行
        # if "android.widget.EditText" in line:
        #     self.line=re.sub(r'text="[^"]*"', 'text=""', line)
    ...
    def get_attribute(self,keywords):
        ...
        flagmarks = line[attributenum+len(keywords):attributenum+len(keywords)+1]
        line=line[attributenum+len(keywords)+1:len(line)-1]
        if flagmarks=="\'":
            specialmarksnum=line.find('\'')
            attribute=line[0:specialmarksnum]
        else:
            marksnum=line.find('\"')
    ...
    def same(self,view):
        ...
        if self.scrollable !="#any#" and view.scrollable !="#any#" and self.scrollable != view.scrollable:
            return False
        #很有可能出问题
        if self.x !="#any#" and view.x !="#any#" and self.x !=-1 and view.x !=-1 and self.x !="" and view.x !="" and self.x != view.x:
            return False
        if self.y !="#any#" and view.y !="#any#" and self.y !=-1 and view.y !=-1 and self.y !="" and view.y !="" and self.y != view.y:
            return False
        return True
